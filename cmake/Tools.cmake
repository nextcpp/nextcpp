function(add_cmake_format_target)
    ...
    if(CMAKE_FORMAT)
        message(STATUS "cmake-format found -> added")
...
function(add_clang_format_target)
    ...
    file(GLOB_RECURSE CMAKE_FILES_CPPM "*/*.cppm")
    file(GLOB_RECURSE CMAKE_FILES_H "*/*.h")
    file(GLOB_RECURSE CMAKE_FILES_HPP "*/*.hpp")
    file(GLOB_RECURSE CMAKE_FILES_IXX "*/*.ixx")
    set(CPP_FILES
        ${CMAKE_FILES_CC}
        ${CMAKE_FILES_CPP}
        ${CMAKE_FILES_CPPM}
    ...
    if(CLANGFORMAT)
        message(STATUS "clang-format found -> added")
        add_custom_target(
            run_clang_format
            COMMAND
                ${Python3_EXECUTABLE}
...
function(add_clang_tidy_to_target target)
    ...
    list(
        ...
        INCLUDE
        REGEX
        ".*.(cc|cpp|cppm|ixx|h|hpp)")

    find_package(Python3 COMPONENTS Interpreter)
    if(NOT ${Python_FOUND})
        message(WARNING "python3 is need for the clang-tidy scan")
        return()
    endif()
    ...
    if(CLANGTIDY)
        if(CMAKE_CXX_COMPILER_ID MATCHES "MSVC")
            message(STATUS "Added MSVC clang-tidy (VS GUI only) for: ${target}")
            ...
            message(STATUS "clang-tidy set up to go through: ${target}")
            add_custom_target(
                ${target}_clangtidy
                COMMAND
                    ${Python3_EXECUTABLE}
                    ...
                    -header-filter="\(src|app\)\/*.\(ixx|cppm|h|hpp\)"
                    -p=${CMAKE_BINARY_DIR}
